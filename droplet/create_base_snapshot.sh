...
if [ -z "${ID}" ]; then
    ...
    ID=`doctl compute droplet create mbtest --no-header --format "ID" --wait --size 1gb --region fra1 --image 30970148 --ssh-keys f9:60:c8:4e:27:83:ec:59:b9:bb:a6:20:9f:2e:2d:3b`
fi
echo "ID is $ID"
