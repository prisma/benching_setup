...
bench_query() {
    ...
    local WARMUP_RPS="$(echo "$QUERY_PARAMS" | jq -rc '.warmup_rps')"
    for CANDIDATE in $(echo "$QUERY_PARAMS" | jq -rc '.candidates[]'); do
        ...
        if [ "$WARMUP_DURATION" != "null" ]; then
            echo ""
            echo ""
            echo "----------------- Warmup: $QUERY_NAME $PROGRAM_DIR $PROGRAM_URL ${WARMUP_RPS}Req/s ${WARMUP_DURATION}s -----------------"
            echo ""
            run_vegeta "$PROGRAM_URL" "$PROGRAM_DIR" "$QUERY_NAME" "${WARMUP_RPS}" "$WARMUP_DURATION" "temp/tmp.warmup" "$TIMEOUT"
            rm "temp/tmp.warmup"
